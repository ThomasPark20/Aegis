---
name: x-integration
description: X (Twitter) integration for AEGIS. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** AEGIS v1.0.0. Directory structure may change in future versions.
...
1. **AEGIS is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep aegis (macOS) or systemctl --user status aegis (Linux)
```
...
| `AEGIS_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/aegis.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into AEGIS, make the following modifications:
...
All paths below are relative to project root (`AEGIS_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep aegis  # macOS — should show PID and exit code 0 or -
# Linux: systemctl --user status aegis
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/aegis.log | tail -20

# Script errors
grep -i "error\|failed" logs/aegis.log | tail -20
```
...
docker run nanoclaw-agent:latest ls -la /app/src/skills/
```
