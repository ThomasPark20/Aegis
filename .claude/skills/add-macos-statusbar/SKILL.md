...
description: Add a macOS menu bar status indicator for AEGIS. Shows a bolt icon with a green/red dot indicating whether AEGIS is running, with Start, Stop, and Restart controls. macOS only.
---

# Add macOS Menu Bar Status Indicator

Adds a persistent menu bar icon that shows AEGIS's running status and lets the user
...
```bash
launchctl list | grep com.aegis.statusbar
```
...
Create `~/Library/LaunchAgents/com.aegis.statusbar.plist`, substituting the actual values
...
<dict>
    <key>Label</key>
    <string>com.aegis.statusbar</string>
...
```bash
launchctl load ~/Library/LaunchAgents/com.aegis.statusbar.plist
```
...
> The bolt icon should now appear in your macOS menu bar. Click it to see AEGIS's status and control the service.
>
> - **Green dot** — AEGIS is running
> - **Red dot** — AEGIS is stopped
>
...
```bash
launchctl unload ~/Library/LaunchAgents/com.aegis.statusbar.plist
rm ~/Library/LaunchAgents/com.aegis.statusbar.plist
rm dist/statusbar
```
