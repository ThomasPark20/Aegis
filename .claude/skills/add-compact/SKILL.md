...
> **Note:** `upstream` is the remote pointing to `qwibitai/aegis`. If using a different remote name, substitute accordingly.
...
1. Start AEGIS in dev mode: `npm run dev`
...
```bash
git clone <your-fork> /tmp/aegis-test
cd /tmp/aegis-test
...
- **No config file.** AEGIS's philosophy is customization through code changes, not configuration sprawl.
