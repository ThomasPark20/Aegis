...
This skill replaces the OneCLI gateway with AEGIS's built-in credential proxy. Containers get credentials injected via a local HTTP proxy that reads from `.env` — no external services needed.
...
If neither check matches, you may be on an older version. Run `/update-aegis` first, then retry.
...
- macOS: `launchctl kickstart -k gui/$(id -u)/com.aegis`
...
```bash
tail -20 logs/aegis.log | grep "Credential proxy"
```
