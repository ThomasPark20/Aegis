...
<dict>
    ...
    <string>${projectRoot}/logs/aegis.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/aegis.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.aegis');
  } catch {
...
/**
 * Kill any orphaned aegis node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned aegis processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/aegis.service';
    ...
    unitPath = path.join(unitDir, 'aegis.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=AEGIS Personal Assistant
...
StandardOutput=append:${projectRoot}/logs/aegis.log
StandardError=append:${projectRoot}/logs/aegis.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned aegis processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable aegis`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start aegis`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active aegis`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-aegis.sh');
  const pidFile = path.join(projectRoot, 'aegis.pid');

  const lines = [
    '#!/bin/bash',
    '# start-aegis.sh — Start AEGIS without systemd',
    ...
    '    echo "Stopping existing AEGIS (PID $OLD_PID)..."',
    ...
    `  >> ${JSON.stringify(projectRoot + '/logs/aegis.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/aegis.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "AEGIS started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/aegis.log"`,
  ];
